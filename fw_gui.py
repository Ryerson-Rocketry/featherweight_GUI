...
def update_gps_data(n):
    if fw.freq == freq1:
        lat = round(coord_f1[-1][0], 4) # Round to 4 decimal places
        long = round(coord_f1[-1][1], 4) # Round to 4 decimal places
        return html.Div([
        ...
        lat = round(coord_f2[-1][0], 4)  # Round to 4 decimal places
        long = round(coord_f2[-1][1], 4)  # Round to 4 decimal places
        return html.Div([
            html.Div("GPS DATA COLUMN", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'}),
            html.Div("Current Coords:", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'}),
            html.Div(f"Lat:{lat}", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'}),
        ...
        return html.Div([
            html.Div("DATA", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'}),
            html.Div("Current Coords", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'}),
            html.Div(f"Lat:{lat}", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'}),
            html.Div(f"Long:{long}", style={'color': 'white', 'text-align': 'center', 'font-family': 'Roboto'})
        ], style={'line-height': '2', 'text-align': 'center'})
...
def update_map(n,f):
    ...
    if (data!='NoneType' and print_data == True):
        fw.print_gps(data)
        if fw.freq==freq1:
            coord_f1.append(fw.pos_list[-1])
            return [dl.TileLayer(),dl.Polyline(color="#0000ff",positions=coord_f1)]
        elif fw.freq==freq2:
            coord_f2.append(fw.pos_list[-1])
            return [dl.TileLayer(),dl.Polyline(color="#ff0000",positions=coord_f2)]         
        elif fw.freq==0:            
